...
# IF you wanna avoid target leakage completely, defrozen the following consoles
#a = float_data[:, 0]
...
blocks1 = Model(block_input, [backcast, forecast])
blocks2 = Model(block_input, [backcast, forecast])

#******************* Define the stack *********************
# Stack 1
stack_input = Input(batch_shape=(batch_size, backcast_length, num_features), name='stack_input')

b_block_1, f_block_1 = blocks1(stack_input)
x_residual_1 = layers.Subtract(name='x_residual_1')([stack_input, b_block_1])

b_block_2, f_block_2 = blocks1(x_residual_1)
x_residual_2 = layers.Subtract(name='x_residual_2')([x_residual_1, b_block_2])

f_stack_1 = layers.Add(name='f_stack_1')([f_block_1, f_block_2])
b_stack_1 = layers.Add(name='b_stack_1')([b_block_1, b_block_2])

# Stack 2
b_block_1_s2, f_block_1_s2 = blocks2(x_residual_2)
x_residual_3 = layers.Subtract(name='x_residual_3')([x_residual_2, b_block_1_s2])

b_block_2_s2, f_block_2_s2 = blocks2(x_residual_3)
x_residual_4 = layers.Subtract(name='x_residual_4')([x_residual_3, b_block_2_s2])

f_stack_2 = layers.Add(name='f_stack_2')([f_block_1_s2, f_block_2_s2])
b_stack_2 = layers.Add(name='b_stack_2')([b_block_1_s2, b_block_2_s2])

# Real double stacks
stack = Model(stack_input, [b_stack_2, f_stack_2])

#******************* Define TensorBoard *********************
callbacks_list = [TensorBoard(log_dir='my_log_dir',
                              histogram_freq=1)]
#******************* Compile and Fit *********************
...
print(model.summary())
print('*'*100)
model.compile(optimizer=RMSprop(lr=learning_rate), loss='mae', loss_weights=[0.2,0.8])
...
plt.clf()
# *************************************
forecast_loss = hist_dict['f_stack_2_loss']
val_forecast_loss = hist_dict['val_f_stack_2_loss']
epochs = range(1, 1+len(loss))
...
plt.title('Training and Validation double stacks foreast MAE')
plt.legend()
mplcyberpunk.add_glow_effects()
...
plt.clf()
#*************************************
backcast_loss = hist_dict['b_stack_2_loss']
val_backcast_loss = hist_dict['val_b_stack_2_loss']
epochs = range(1, 1+len(loss))
...
plt.title('Training and Validation double stacks backcast MAE')
plt.legend()
mplcyberpunk.add_glow_effects()
...
#******************* Get Res *********************
Resx1 = Model(stack.input, stack.get_layer('x_residual_1').output, trainable=False)
Resx2 = Model(stack.input, stack.get_layer('x_residual_2').output, trainable=False)
Resx3 = Model(stack.input, stack.get_layer('x_residual_3').output, trainable=False)

#******************* Residual Analysis *********************
model.trainable = False
model.save('DoubleStack1.h5')

for s, [b_tar, f_tar] in test_generator:

    samp = tf.convert_to_tensor(s, dtype_hint = np.float32)
    flag=K.is_keras_tensor(samp)
    print(flag)
    print(samp.shape, 'sampe.shape')
    print(b_tar.shape, 'b_tar.shape')
    print(f_tar.shape, 'f_tar.shape')
    print(type(samp))
    resx1 = K.eval(Resx1(samp))
    resx2 = K.eval(Resx2(samp))
    resx3 = K.eval(Resx3(samp))
    with open("XResidual.csv", "w") as csvfile:
        for index1 in range(batch_size):
            print('The res change of %s-th testing sample' % index1)
            for index2 in range(backcast_length):
                line1 = resx1[index1, index2, :]
                line2 = resx2[index1, index2, :]
                line3 = resx3[index1, index2, :]
                line1 = np.reshape(line1, newshape=(num_features,))
                line2 = np.reshape(line2, newshape=(num_features,))
                line3 = np.reshape(line3, newshape=(num_features,))
...
'''k = 0
for samp, tar in test_generator:
    if k>=1:
        break
    k+=1
    resx1 = Resx1.predict(samp)
    resx2 = Resx2.predict(samp)
    resx3 = Resx3.predict(samp)
    resx4 = Resx4.predict(samp)

    for index in range(batch_size):
        print('The res change of %s-th testing sample'%index)
        print(resx1[index])
        print(resx2[index])
        print(resx3[index])
        print(resx4[index])
