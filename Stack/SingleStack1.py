...
b_block_1, f_block_1 = block(stack_input)
x_residual_1 = layers.Subtract(name='x_residual_1')([stack_input, b_block_1])

b_block_2, f_block_2 = block(x_residual_1)
x_residual_2 = layers.Subtract(name='x_residual_2')([x_residual_1, b_block_2])

f_stack = layers.Add(name='stack_forecast')([f_block_1, f_block_2])
b_stack = layers.Add(name='stack_backcast')([b_block_1, b_block_2])

stack = Model(stack_input, [b_stack, f_stack])
...
print(model.summary())
print('*'*100)
model.compile(optimizer=RMSprop(lr=learning_rate), loss='mae', loss_weights=[0.01, 1])
...
plt.clf()
# *************************************
forecast_loss = hist_dict['stack_forecast_loss']
val_forecast_loss = hist_dict['val_stack_forecast_loss']
epochs = range(1, 1+len(loss))
...
plt.clf()
#*************************************
backcast_loss = hist_dict['stack_backcast_loss']
val_backcast_loss = hist_dict['val_stack_backcast_loss']
epochs = range(1, 1+len(loss))
